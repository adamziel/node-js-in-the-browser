...
function pushCommonFlags(flags, options, keys) {
	...
	if (dropLabels)
		flags.push(
			`--drop-labels=${validateAndJoinStringArray(
				dropLabels,
				'drop label'
			)}`
		);
	...
	if (define) {
		for (let key in define) {
			...
			flags.push(
				`--define:${key}=${validateStringValue(
					define[key],
					'define',
					key
				)}`
	...
	if (logOverride) {
		for (let key in logOverride) {
			...
			flags.push(
				`--log-override:${key}=${validateStringValue(
					logOverride[key],
					'log override',
					key
				)}`
...
function flagsForBuildOptions(
	...
	if (mainFields)
		flags.push(
			`--main-fields=${validateAndJoinStringArray(
				mainFields,
				'main field'
			)}`
		);
	if (conditions)
		flags.push(
			`--conditions=${validateAndJoinStringArray(
				conditions,
				'condition'
			)}`
		);
	...
	if (banner) {
		for (let type in banner) {
			...
			flags.push(
				`--banner:${type}=${validateStringValue(
					banner[type],
					'banner',
					type
				)}`
	...
	if (footer) {
		for (let type in footer) {
			...
			flags.push(
				`--footer:${type}=${validateStringValue(
					footer[type],
					'footer',
					type
				)}`
	...
	if (loader) {
		for (let ext in loader) {
			...
			flags.push(
				`--loader:${ext}=${validateStringValue(
					loader[ext],
					'loader',
					ext
				)}`
	...
	if (outExtension) {
		for (let ext in outExtension) {
			...
			flags.push(
				`--out-extension:${ext}=${validateStringValue(
					outExtension[ext],
					'out extension',
					ext
				)}`
		...
		if (Array.isArray(entryPoints)) {
			for (let i = 0, n = entryPoints.length; i < n; i++) {
				...
				if (typeof entryPoint === 'object' && entryPoint !== null) {
					...
					if (input === void 0)
						throw new Error(
							'Missing property "in" for entry point at index ' +
								i
						);
					if (output === void 0)
						throw new Error(
							'Missing property "out" for entry point at index ' +
								i
						);
...
function createChannel(streamIn) {
	...
	let readFromStdout = (chunk) => {
		console.error('[esbuild-channel] readFromStdout enter', { chunk });
		const length = chunk && (chunk.length || chunk.byteLength || 0);
		try {
			const ctor =
				chunk && typeof chunk === 'object' && chunk.constructor
					? chunk.constructor.name
					: typeof chunk;
			console.error(
				'[esbuild-channel] chunk meta',
				typeof chunk,
				ctor,
				length
			);
		...
		try {
			const buffer =
				typeof Buffer !== 'undefined' ? Buffer.from(chunk) : null;
			if (buffer) {
				preview = buffer.toString('hex').slice(0, 200);
				if (preview.startsWith('5b766974655d20636f6e6e65637465642e')) {
					console.error(
						'[esbuild-channel] skipping log chunk (vite status)',
		...
		console.error(
			'[esbuild-channel] readFromStdout chunk',
			length,
			preview
		);
	...
	let afterClose = (error) => {
		console.error('[esbuild-channel] afterClose', {
	...
	const channelDebugEnabled =
		...
		process?.env?.ESBUILD_CHANNEL_DEBUG === '1';
	const appendChannelLog = (label, payload) => {
		if (!channelDebugEnabled) return;
		try {
			const fs = require('fs');
			const normalized =
			...
			fs.appendFileSync(
				...
				`${new Date().toISOString()} ${label} ${normalized}\n`,
				'utf8'
			);
			try {
				if (
					...
					const text =
						...
						label +
						' ' +
						(normalized.slice ? normalized.slice(0, 400) : normalized) +
						'\n';
					if (process.stderr) {
	...
	let sendRequest = (refs, value, callback) => {
		appendChannelLog('sendRequest', {
			command: value && value.command,
			argsCount: value && value.args ? value.args.length : 0,
		});
		appendChannelLog('sendRequest.value', value);
	...
	let sendResponse = (id, value) => {
		appendChannelLog('sendResponse', {
			id,
			keys: value ? Object.keys(value) : [],
		});
		appendChannelLog('sendResponse.value', value);
		if (closeData.didClose)
			throw new Error(
				'The service is no longer running' + closeData.reason
			);
	...
	let handleRequest = async (id, request) => {
		appendChannelLog('handleRequest', {
			id,
			command: request && request.command,
			argsCount: request && request.args ? request.args.length : 0,
		});
		try {
	...
	let handleIncomingPacket = (bytes) => {
		const rawHex =
			typeof Buffer !== 'undefined'
				...
				: Array.from(bytes)
						...
						.join('');
		appendChannelLog('incoming.raw', {
			length: bytes ? bytes.length : 0,
			hex: rawHex.length > 800 ? rawHex.slice(0, 800) + '…' : rawHex,
		});
		if (isFirstPacket) {
			isFirstPacket = false;
			...
			if (binaryVersion !== '0.25.11') {
				throw new Error(
					`Cannot start service: Host version "${'0.25.11'}" does not match binary version ${quote(
						binaryVersion
					)}`
				);
			}
			appendChannelLog('incoming.handshake', { binaryVersion });
			return;
		}
		let packet;
		try {
			...
			appendChannelLog('incoming.decode.error', {
		...
		appendChannelLog('incoming.decoded', {
			id: packet.id,
			isRequest: packet.isRequest,
			type:
				packet.value && typeof packet.value === 'object'
					? packet.value.command || '<no-command>'
					: typeof packet.value,
		});
		if (packet.isRequest) {
			...
			appendChannelLog('handleResponse', {
				id: packet.id,
				keys: packet.value ? Object.keys(packet.value) : [],
			});
			appendChannelLog('handleResponse.value', packet.value);
	...
	function buildOrContextContinue(
		...
		const buildResponseToResult = (response, callback2) => {
			...
			if (response.outputFiles)
				result.outputFiles =
					response.outputFiles.map(convertOutputFiles);
			...
			const result = {
				...
				serve: (options2 = {}) =>
					new Promise((resolve, reject) => {
						...
						const servedir = getFlag(
							options2,
							keys,
							'servedir',
							mustBeString
						);
						const keyfile = getFlag(
							options2,
							keys,
							'keyfile',
							mustBeString
						);
						const certfile = getFlag(
							options2,
							keys,
							'certfile',
							mustBeString
						);
						const fallback = getFlag(
							options2,
							keys,
							'fallback',
							mustBeString
			...
			let resolve = (path3, options = {}) => {
				...
				return new Promise((resolve2, reject) => {
					...
					if (importAttributes != null)
						request.with = sanitizeStringMap(
							importAttributes,
							'with'
						);
					sendRequest(refs, request, (error, response) => {
				...
				try {
					...
					if (result != null) {
						if (typeof result !== 'object')
							throw new Error(
								`Expected onStart() callback in plugin ${quote(
									name
								)} to return an object`
							);
						let keys = {};
		...
		for (let id2 of request.ids) {
			try {
				...
				if (result != null) {
					if (typeof result !== 'object')
						throw new Error(
							`Expected onResolve() callback in plugin ${quote(
								name
							)} to return an object`
						);
					let keys = {};
		...
		for (let id2 of request.ids) {
			try {
				...
				if (result != null) {
					if (typeof result !== 'object')
						throw new Error(
							`Expected onLoad() callback in plugin ${quote(
								name
							)} to return an object`
						);
					let keys = {};
					...
					try {
						...
						if (value != null) {
							if (typeof value !== 'object')
								throw new Error(
									`Expected onEnd() callback in plugin ${quote(
										name
									)} to return an object`
								);
							let keys = {};
			...
			while (true) {
				...
				if (match) {
					...
					return {
						...
						length: encodeUTF8(
							lineText.slice(column, column + length)
						).length,
						lineText: lineText + '\n' + lines.slice(1).join('\n'),
						suggestion: '',
					};
...
var isInternalWorkerThread =
	((_a = worker_threads == null ? void 0 : worker_threads.workerData) == null
		? void 0
		: _a.esbuildVersion) === '0.25.11';
var esbuildCommandAndArgs = () => {
	if (
		(!ESBUILD_BINARY_PATH || true) &&
...
var ensureServiceIsRunning = () => {
	...
	// let child = child_process.spawn(command, args.concat(`--service=${"0.25.11"}`, "--help"), {
	//   windowsHide: true,
	...
	let child = child_process.spawn(
		command,
		args.concat(`--service=${'0.25.11'}`, '--ping'),
		{
			windowsHide: true,
			stdio: ['pipe', 'pipe', 'inherit'],
			cwd: defaultWD,
	...
	writeToStdin(bytes) {
			console.error('[esbuild-channel] writeToStdin bytes', bytes);
			const length = bytes && (bytes.length || bytes.byteLength || 0);
			let preview = null;
			try {
				preview =
					typeof Buffer !== 'undefined'
						? Buffer.from(bytes).toString('hex').slice(0, 200)
						: null;
			} catch {
				preview = null;
			}
			console.error('[esbuild-channel] writeToStdin', length, preview);
			try {
				if (bytes && bytes.length >= 4) {
					const packetLength = readUInt32LE(bytes, 0);
					const payload = bytes.subarray(4, 4 + packetLength);
					const decoded = decodePacket(payload);
					let decodedSummary = decoded;
					try {
						decodedSummary =
					...
					appendChannelLog('sendPacket.decoded', decodedSummary);
					console.error(
						'[esbuild-channel] writeToStdin decoded',
						decodedSummary &&
							typeof decodedSummary === 'object' &&
							decodedSummary.value &&
							typeof decodedSummary.value === 'object'
							? Object.assign(
									{},
									decodedSummary,
									{
										value: Object.assign(
											...
											decodedSummary.value,
											{
												input:
													decodedSummary.value
														.input &&
													decodedSummary.value.input
														.length
														? `[bytes:${decodedSummary.value.input.length}]`
														: decodedSummary.value.input,
											}
										),
									}
				...
				console.error(
					'[esbuild-channel] writeToStdin decode error',
					decodedError && decodedError.message
				);
			}
	...
	stdout.on('data', (chunk) => {
		console.error('[esbuild-channel] stdout data', chunk);
		readFromStdout(chunk);
	});
	stdout.on('end', () => {
		console.error('stdout end on child closed');
		afterClose();
	});
	stopService = () => {
		stdin.destroy();
...
error: ${text}`);
	...
	let runCallSync = (command, args) => {
		...
		if (id !== id2)
			throw new Error(
				`Internal error: Expected id ${id} but got id ${id2}`
			);
		if (reject) {
				...
				try {
					switch (command) {
						...
						case 'transform':
							workerPort.postMessage({
								id,
								resolve: await service.transform(
									args[0],
									args[1]
								),
							});
						...
						case 'formatMessages':
							workerPort.postMessage({
								id,
								resolve: await service.formatMessages(
									args[0],
									args[1]
								),
							});
						...
						case 'analyzeMetafile':
							workerPort.postMessage({
								id,
								resolve: await service.analyzeMetafile(
									args[0],
									args[1]
								),
							});
