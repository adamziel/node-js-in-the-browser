...
type SyncWrapEndpoint =
	| IsomorphicMessagePort
	| Endpoint
	| Worker
	| (Endpoint & { terminate?: () => void });

const endpointPortPromises = new WeakMap<Endpoint, Promise<MessagePort>>();
const endpointHandshakeTimeoutMs = 5000;

function isMessagePortLike(
	value: SyncWrapEndpoint
): value is IsomorphicMessagePort {
	return (
		!!value &&
...
export function wrapSync<T>(
	...
	async function doWrap() {
		...
		await NodeSABSyncReceiveMessageTransport.ensureBrowserConnection(port);
		transport = await NodeSABSyncReceiveMessageTransport.create();
		console.log({transport})
		return createSyncProxy<T>(
...
export class NodeSABSyncReceiveMessageTransport {
	...
	private static environment: 'node' | 'browser' | undefined;
...
listen(topLevelTarget, (data) => {
	...
	try {
		...
		console.log("init message handled");
	} catch (error) {
...
function handleInitMessage(data) {
	...
	listen(commandPort, (messageData) => {
		console.log("[PUMP] commandPort message received", {messageData});
	...
	listen(port, (payloadData) => {
		console.log("[PUMP] port message received", {payloadData});
		...
		while (offset < bytes.length) {
			...
			offset += chunkSize
			console.log("[PUMP] port message sent", {payload, chunkSize});
		}
	})
...
`;
	...
	static async create() {
		if (!NodeSABSyncReceiveMessageTransport.environment) {
			NodeSABSyncReceiveMessageTransport.environment =
				NodeSABSyncReceiveMessageTransport.detectEnvironment();
		}

		if (
			NodeSABSyncReceiveMessageTransport.environment === 'node' &&
			...
			NodeSABSyncReceiveMessageTransport.environment === 'browser'
		) {
			NodeSABSyncReceiveMessageTransport.assertBrowserSupport();
		}
	...
	send(
		...
		if (NodeSABSyncReceiveMessageTransport.environment === 'browser') {
			const connection = NodeSABSyncReceiveMessageTransport.browserConnections.get(
				ep as MessagePort
			);
			if (!connection) {
				throw new Error("Connection with the worker not found");
			}

			console.log("sending in browser", {connection});
			return this.sendInBrowser(
				connection,
				msg,
				transferables ?? []
			);
		}
		return this.sendInNode(ep, msg, transferables ?? []);
	}
	...
	private sendInBrowser(
		...
		while (true) {
			...
			const res =
				NodeSABSyncReceiveMessageTransport.browserReceiveMessageOnPort(
					connection
				);
	...
	private static detectEnvironment(): 'node' | 'browser' {
		if (
	...
	private static assertBrowserSupport() {
	...
	private static async ensureBrowserConnection(
		...
		const worker =
			NodeSABSyncReceiveMessageTransport.createPumpWorker();
		...
		await new Promise<void>((resolve) => {
			// @TODO – resolve when worker replies
		...
		const waitResult = Atomics.wait(
			...
			0,
			0,
			NodeSABSyncReceiveMessageTransport.handshakeTimeoutMs
		);
		...
		NodeSABSyncReceiveMessageTransport.browserConnections.set(
			port,
			connection
		);
		return connection;
	...
	private static getPumpWorkerUrl(): string {
		if (!NodeSABSyncReceiveMessageTransport.browserPumpWorkerUrl) {
			const blob = new Blob(
				[NodeSABSyncReceiveMessageTransport.pumpWorkerSource],
				{ type: 'text/javascript' }
			);
			NodeSABSyncReceiveMessageTransport.browserPumpWorkerUrl =
				URL.createObjectURL(blob);
		}
		return NodeSABSyncReceiveMessageTransport.browserPumpWorkerUrl;
	}
	...
	private static createPumpWorker(): Worker {
		...
		if (
			...
			return new Worker(
				NodeSABSyncReceiveMessageTransport.getPumpWorkerUrl(),
				{
					name: 'comlink-sync-pump',
				}
			);
	...
	private static browserReceiveMessageOnPort(
		...
		for (;;) {
			const cur = Atomics.load(connection.control, 0);
			if (cur === STATE_FULL) {
				...
				if ((flags & FLAG_MORE) === 0) {
					...
					const decoder =
						NodeSABSyncReceiveMessageTransport.textDecoder ||
						new TextDecoder();
					NodeSABSyncReceiveMessageTransport.textDecoder = decoder;
			...
			if (options?.timeoutMs !== undefined) {
				...
				Atomics.wait(connection.control, 0, cur);
			}
		}
	}
}
